...
Процедура ТаблицаНагрузкиПередУдалением(Элемент, Отказ)
	...
	Если Не ДанныеСтроки.УчетДолжностей Тогда
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьНагрузкуИСтавкуДоИзменения()
	...
	Если Объект.Первичный Тогда
		...
		|	ЗНАЧЕНИЕ(Справочник.ВидыЗаписейШтатногоРасписания.ПустаяСсылка) КАК ВидЗаписи,
	...
	Иначе
		Запрос.Текст = "ВЫБРАТЬ
		|	ЕСТЬNULL(ШтатноеРасписание.ВидЗаписи, ЗНАЧЕНИЕ(Справочник.ВидыЗаписейШтатногоРасписания.ПустаяСсылка)) КАК ВидЗаписи,
